...
SUSPICIOUS_PATTERNS = [
    # === SQL Injection ===
    (
        r"(?i)\b(select|insert|update|delete|drop|truncate|create|alter|exec|execute)\b",
        2,
        "SQL keyword",
    ),
    (r"(?i)\b(select|union).+?(from|where|join)", 4, "SQL clause structure"),
    (
        r"(?i)('|\")\s*(or|and)\s*['\"]?(1|a|a')\s*=\s*['\"]?(1|a|a')",
        4,
        "Classic SQLi tautology",
    ),
    (
        r"(?i);(?:\s*(drop|delete|shutdown|exec|system|cmd))",
        4,
        "Stacked query + dangerous command",
    ),
    (r"--|#|/\*.*?\*/", 3, "SQL comment"),
    # === XSS ===
    ...
    (
        r"(rm|cat|ls|ps|whoami|nc|bash|sh|wget|curl)\s*[-/]",
        3,
        "Suspicious command with flags",
    ),
    # === Path Traversal ===
    (r"\.\./|\.\.\\", 3, "Path traversal"),
...
def calculate_total_score(text: str) -> Tuple[int, List[Tuple[str, int, str]]]:
    ...
    has_sql_kw = any(
        re.search(p, normalized, re.I)
        for p in [r"\b(update|drop|select|insert|delete)\b"]
    )
...
def sanitize_input(
    ...
    *,
    ...
    context: Optional[Dict[str, Any]] = None,  # Fixed: Optional type hint
    ...
    if detect_script_mix(normalized_text):
        ...
        matched_patterns.append(
            ("mixed_script", 3, "Possible homoglyph attack (mixed scripts)")
        )
    ...
    if log_suspicious and total_score > 0:
        ...
        if context:  # This now handles None properly
